  ...
  useEffect(()=>{
    GetFreightImages()
     ...
     const GetFreightImages = () => {
      const data1 = { freight_id:data.id,uploaded_by:"2" };
    
      axios
        .post(`${process.env.REACT_APP_BASE_URL}GetFreightImages`, data1)
        .then((response) => {
                    ...
                    <div key={groupIndex} className="mb-2">
                      ...
                      {documents[groupName]?.map((item, index) => (
                        <div key={item.id} className="d-flex align-items-center">
                          <a
                            href={`${process.env.REACT_APP_BASE_URL}document/${item?.document}`}
                            target="_blank"
